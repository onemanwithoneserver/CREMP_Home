...
export default function FAQDesktop() {
    ...
    return (
        <section className="w-full bg-[#FAFAFA] py-12 px-6 relative overflow-hidden">
            {/* Background Accents */}
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
                <div className="absolute -top-[20%] -right-[10%] w-[600px] h-[600px] bg-[#d4af37]/5 rounded-full blur-[120px]" />
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            >
                                {faqData.questions.map((q, index) => (
                                    ...
                                    >
                                        <div className={`w-1 h-full absolute left-0 top-0 z-20 bg-gradient-to-b from-[#bf953f] to-[#b38728] transition-opacity duration-300 ${expandedId === q.id ? 'opacity-100' : 'opacity-0'}`} />
                                        
                                        <button
                                            onClick={() => setExpandedId(expandedId === q.id ? null : q.id)}
                                            className="w-full flex items-start justify-between p-4 text-left relative z-10 bg-white"
                                        >
                                            <span className={`text-lg font-semibold pr-8 transition-colors duration-300 ${expandedId === q.id ? "text-[#d4af37]" : "text-[#0a1128] group-hover:text-[#0a1128]"}`}>
                                                {q.question}
                                            </span>
                                            <motion.div
