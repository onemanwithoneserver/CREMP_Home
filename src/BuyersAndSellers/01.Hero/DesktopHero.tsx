...
const staggerContainer: Variants = { hidden: { opacity: 0 }, show: { opacity: 1, transition: { staggerChildren: 0.15, delayChildren: 0.2 } } };
const fadeInUp: Variants = { hidden: { opacity: 0, y: 30 }, show: { opacity: 1, y: 0, transition: { type: 'spring', stiffness: 400, damping: 30 } } };
const popIn: Variants = { hidden: { opacity: 0, scale: 0.9 }, show: { opacity: 1, scale: 1, transition: { type: 'spring', stiffness: 400, damping: 25 } } };
    ...
    <div className="relative min-h-[90vh] w-full overflow-hidden bg-[#030712] flex flex-col items-center justify-center py-24">
      ...
      <Container className="relative z-10 max-w-7xl px-4 xl:px-0">
        <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-col items-center text-center mt-8">
          ...
          <motion.p variants={fadeInUp} className="max-w-2xl text-[1.1rem] font-medium leading-relaxed text-gray-400 mb-10">
            {heroData.desc}
          </motion.p>
          ...
          <motion.div variants={fadeInUp} className="flex items-center gap-5">
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="group flex items-center gap-3 rounded-[8px] border border-white/20 bg-white/5 px-8 py-4 text-[0.95rem] font-bold text-white backdrop-blur-md transition-all hover:bg-white/10 hover:border-white/30">
              <SecondaryIcon size={20} className="text-gray-300 transition-colors group-hover:text-white" />
              {heroData.buttons[1].text}
              <ArrowRight size={16} className="ml-2 opacity-0 -translate-x-4 transition-all group-hover:opacity-100 group-hover:translate-x-0" />
            </motion.button>
          </motion.div>
        ...
        <motion.div variants={popIn} initial="hidden" animate="show" transition={{ delay: 1 }} className="mx-auto mt-16 w-full max-w-4xl rounded-[8px] border border-white/10 bg-white/5 p-6 backdrop-blur-xl shadow-2xl">
