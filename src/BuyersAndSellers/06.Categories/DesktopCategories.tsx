  ...
  return (
    <div className="relative w-full overflow-hidden bg-gray-50 py-16 transition-colors duration-700 dark:bg-[#030712]">
      <Container className="relative z-10 max-w-7xl px-4 xl:px-0">
        ...
        <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-50px" }} className="grid grid-cols-5 gap-6">
          {categoriesData.categories.map((cat, idx) => (
            ...
            >
              ...
              <div className="relative z-10">
                <h3 className="whitespace-pre-line text-lg font-black text-gray-900 transition-colors group-hover:text-white dark:text-white">{cat.label}</h3>
              </div>
            </motion.div>
          ))}
        </motion.div>

        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.6 }} className="mt-8 text-center text-[0.85rem] font-medium text-gray-400">
