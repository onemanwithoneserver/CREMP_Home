...
import { ChevronDown, MapPin, SlidersHorizontal, Filter, Layers, Tag, Handshake, Users, Construction, Briefcase, CheckCircle2, Maximize, CircleDollarSign, Building2, Store } from "lucide-react";
...
export default function FilterDropdownDesktop({ activeTab, filters, onFilterChange, onClose }: FilterDropdownProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -10, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.98 }} transition={{ duration: 0.2, ease: "easeOut" }} className="absolute top-[calc(100%+8px)] right-0 bg-white rounded-2xl shadow-[0_16px_48px_rgba(0,0,0,0.18)] border border-gray-100 overflow-hidden z-[9999] flex flex-col w-full max-w-[440px]">
      ...
      <div className="px-4 pt-4 pb-2 shrink-0 flex items-center justify-between">
        ...
        <button type="button" onClick={handleReset} className="text-[11px] font-bold text-[#0a1128] hover:text-[#d4af37] transition-colors underline underline-offset-2">
          Reset
        </button>
      </div>

      <div className="flex-1 overflow-y-auto scrollbar-hide px-4 pb-4 space-y-4 max-h-[calc(100vh-200px)]">
        ...
        {activeTab === "commercial" ? (
          <div className="space-y-2">
            <div className="bg-gray-50/80 border border-gray-200/70 shadow-sm rounded-xl overflow-hidden">
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <div className="px-3 pb-3 grid grid-cols-3 gap-2">
                      {PROPERTY_TYPES.map((pt) => {
                        ...
                        return (
                          <button key={pt.id} onClick={() => handleUpdate("propertyType", filters.propertyType === pt.id ? "Any" : pt.id)} className={clsx("flex flex-col items-center justify-center p-2 rounded-xl border transition-all h-[68px] gap-1 hover:shadow-sm", isSelected ? "bg-[#0a1128] border-[#0a1128] shadow-[0_4px_12px_rgba(10,17,40,0.15)]" : "bg-white border-gray-200 shadow-sm hover:border-gray-300")}>
          ...
          <AnimatePresence>
            {advancedOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }} className="overflow-hidden">
                <div className="mt-3 rounded-xl bg-gray-50/80 border border-gray-200/70 shadow-inner p-2.5 space-y-0.5">
                  
                  {activeTab === "commercial" && (
                    <div className="flex flex-col">
                      <button type="button" onClick={() => toggleCategory("size")} className={clsx("w-full flex items-center justify-between px-2.5 py-2.5 rounded-lg transition-colors", expandedCategoryKey === "size" ? "bg-white shadow-sm" : "hover:bg-white")}>
                  ...
                  {advancedCategories.map((cat) => {
                    ...
                    return (
                      <div key={cat.key} className="flex flex-col">
                        <button type="button" onClick={() => toggleCategory(cat.key)} className={clsx("w-full flex items-center justify-between px-2 py-2 rounded-lg transition-colors", isExpanded ? "bg-white shadow-sm" : "hover:bg-white")}>
      ...
      <div className="px-4 py-3 border-t border-gray-100 bg-white shrink-0 rounded-b-2xl">
        <button type="button" onClick={onClose} className="w-full py-2 text-[12px] font-bold text-white bg-[#d4af37] rounded-lg hover:bg-[#c19b2e] transition-colors shadow-[0_4px_12px_rgba(212,175,55,0.4)] text-center flex flex-col items-center leading-tight">
