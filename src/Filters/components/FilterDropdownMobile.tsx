...
import { ChevronDown, MapPin, SlidersHorizontal, Filter, Layers, Tag, Handshake, Users, Construction, Briefcase, CheckCircle2, Maximize, IndianRupee, Building2, Store, X, Plus, Minus, Check } from "lucide-react";
...
export default function FilterDropdownMobile({ activeTab, filters, onFilterChange, onClose }: FilterDropdownProps) {
  ...
  const businessAdvancedCategories: CategoryDef[] = [
    { key: "businessOption", title: "Business Options", subtitle: "Business Options", icon: Briefcase, iconBg: "bg-[#ea580c] text-white", options: BUSINESS_OPTIONS.map(o => ({ id: o.id, label: o.label })) },
  ];
                      ...
                      <div className="px-3 pb-3 grid grid-cols-3 gap-2">
                        {PROPERTY_TYPES.map((pt) => {
                          ...
                          return (
                            ...
                            >
                              <pt.icon className={clsx("w-4 h-4", isSelected ? "text-[#d4af37]" : "text-gray-500")} strokeWidth={1.5} />
                              <span className={clsx("text-[10px] font-bold text-center leading-tight", isSelected ? "text-white" : "text-gray-700")}>{pt.label}</span>
                            </button>
                          );
                        })}
                      </div>
                      ...
                      <div className="px-3 pb-3 grid grid-cols-2 gap-2">
                        {BOP_INDUSTRIES.map((ind) => {
                          ...
                          return (
                            ...
                            >
                              <ind.icon className={clsx("w-5 h-5", isSelected ? "text-[#d4af37]" : ind.color)} />
                              <span className={clsx("text-[11px] font-bold text-center leading-tight", isSelected ? "text-white" : "text-gray-700")}>{ind.label}</span>
                            </button>
                          );
                        })}
                      </div>
                    ...
                    {activeTab === "commercial" && (
                      <div className={clsx("flex flex-col", expandedCategoryKey === "size" ? "overflow-visible z-30 relative" : "overflow-hidden")}>
                        ...
                        >
                          ...
                          <ChevronDown className={clsx("w-4 h-4 text-gray-400 transition-transform", expandedCategoryKey === "size" ? "rotate-180" : "")} />
                        </button>
                        <AnimatePresence>
                          {expandedCategoryKey === "size" && (
                            <motion.div
                                    ...
                                    cat.options.map((opt) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          >
                                            {isSelected && <Check className="w-3 h-3 stroke-[3] text-[#d4af37]" />}
                                          </div>
