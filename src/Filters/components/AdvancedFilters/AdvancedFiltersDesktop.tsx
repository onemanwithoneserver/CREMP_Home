...
export default function AdvancedFiltersDesktop({ activeTab, filters, onFilterChange, onClose }: AdvancedFiltersProps) {
  ...
  return (
    <>
      <motion.div
        ...
        exit={{ opacity: 0 }}
        onClick={onClose}
        className="fixed inset-0 bg-black/60 z-[99998] hidden sm:block backdrop-blur-sm"
      />

      <div className="fixed inset-0 z-[99999] pointer-events-none hidden sm:flex items-center justify-center p-4">
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 10 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 10 }}
          className="bg-[#f9fafb] rounded-2xl shadow-2xl w-full max-w-[500px] flex flex-col pointer-events-auto max-h-[90vh]"
        >
          {/* Header */}
          <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-white rounded-t-2xl shrink-0">
            <button
              onClick={onClose}
            ...
            <div className="text-center">
              ...
              <p className="text-[13px] text-gray-500 font-medium mt-0.5">Refine your {activeTab} search</p>
            </div>
            <button
                    ...
                    {BUSINESS_OPTIONS.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          className={clsx(
                            "px-5 py-3 rounded-xl text-[14px] font-bold border transition-colors flex items-center gap-2.5",
                            isSelected
                              ? "bg-[#0b1b42] text-[#d4af37] border-[#0b1b42]"
                              : "bg-white text-gray-700 border-gray-200 hover:bg-gray-50 hover:border-gray-300"
                          )}
          ...
          {/* Footer */}
          <div className="p-5 bg-white border-t border-gray-100 rounded-b-2xl shrink-0">
            <div className="flex items-center gap-4 mb-4">
              <button
                onClick={handleResetAll}
                className="px-6 py-3.5 text-[15px] font-bold text-[#0a1128] bg-white rounded-xl border border-gray-200 hover:bg-gray-50 hover:border-gray-300 transition-colors flex-1 text-center"
              >
                Reset All
              </button>
              <button
                onClick={handleApply}
                className="px-6 py-3 text-[15px] font-bold text-white bg-amber-500 hover:bg-amber-600 rounded-xl flex-[2] text-center shadow-md flex flex-col items-center leading-tight transition-colors"
              >
