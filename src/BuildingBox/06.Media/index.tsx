                ...
                <div className="flex items-center justify-between p-4 border-b border-gray-100/80">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <h2 className="text-[1.05rem] font-bold text-[#0a1128] tracking-tight leading-tight">{mediaData.title}</h2>
                            <span className="text-[0.65rem] text-gray-400 font-medium">Photos, Videos & Tours</span>
                        </div>
                    </div>
                    <motion.button
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        className="flex items-center gap-1.5 text-gray-500 hover:text-[#d4af37] transition-colors text-[0.72rem] font-semibold px-3 py-2 rounded-xl border border-gray-200/80 hover:border-[#d4af37]/30 bg-gray-50/50 hover:bg-[#d4af37]/[0.04] shadow-sm"
                    >
                ...
                <div className="flex w-full border-b border-gray-100/80 bg-gray-50/60 backdrop-blur-sm p-1.5 gap-1">
                    {mediaData.tabs.map((tab) => (
                        <button
                            ...
                            className={`relative flex-1 flex flex-col items-center justify-center py-2.5 gap-0.5 transition-all duration-300 rounded-lg z-10 ${
                                activeTab === tab.label
                                    ? "text-white"
                        ...
                        >
                            ...
                            <span className="text-[0.6rem] font-bold whitespace-nowrap relative z-10 tracking-wide">{tab.label}</span>
                            <span className={`text-[0.52rem] font-semibold relative z-10 ${activeTab === tab.label ? 'opacity-60' : 'text-gray-300'}`}>{tab.count}</span>
                        </button>
                    ))}
                </div>
                        ...
                        >
                                ...
                                <motion.div variants={staggerContainer} initial="hidden" animate="show" className="flex flex-wrap gap-2.5">
                                    {mediaData.videos.map((video, idx) => (
                                        <motion.div
                                            ...
                                            className={`relative rounded-xl overflow-hidden group cursor-pointer shadow-sm hover:shadow-lg transition-shadow duration-500 ${video.fullWidth ? 'w-full h-44' : 'w-[calc(50%-0.3125rem)] h-28'}`}
                                        >
                                            ...
                                            <div className="absolute bottom-2.5 left-2.5 right-2.5">
                                                <span className="bg-[#0a1128]/80 backdrop-blur-xl text-white text-[0.58rem] font-semibold px-2.5 py-1 rounded-lg truncate block w-fit max-w-full border border-white/10">
                                    ...
                                    {mediaData.floorPlans.map((plan, idx) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    <span className="text-[0.85rem] font-semibold text-[#0a1128]">{plan.title}</span>
                                                    <span className="text-[0.65rem] text-gray-400 font-medium">{plan.desc}</span>
                                                </div>
                                            </div>
                            ...
                            {activeTab === "Virtual Tour" && (
                                <motion.div
                                    ...
                                    className="relative rounded-xl overflow-hidden w-full h-56 group shadow-lg"
                                >
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-4 text-center">
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <h3 className="text-white text-lg font-bold tracking-tight">{mediaData.virtualTour.title}</h3>
                                            <p className="text-gray-300 text-xs font-medium">{mediaData.virtualTour.desc}</p>
                                        </div>
                                        <motion.button
                                            whileHover={{ scale: 1.05 }}
                                            whileTap={{ scale: 0.95 }}
                                            className="mt-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-[#bf953f] via-[#d4af37] to-[#b38728] hover:from-[#d4af37] hover:via-[#bf953f] hover:to-[#a67c00] text-white font-bold text-sm flex items-center gap-2 transition-all shadow-[0_0_25px_rgba(212,175,55,0.35)] border border-[#f9df9f]/50"
                                        >
                                ...
                                <motion.div variants={staggerContainer} initial="hidden" animate="show" className="grid grid-cols-3 gap-2">
                                    {mediaData.photos.map((photo, idx) => (
                                        <motion.div
                                            ...
                                            className="relative rounded-xl overflow-hidden aspect-square group cursor-pointer shadow-sm hover:shadow-lg transition-shadow duration-500"
                                        >
                                            ...
                                            <div className="absolute bottom-2 left-2">
                                                <span className="bg-[#0a1128]/80 backdrop-blur-xl text-white text-[0.52rem] font-semibold px-2 py-0.5 rounded-lg border border-white/10">
                                                    {photo.label}
                                                </span>
                                            </div>
                                        </motion.div>
                                    ...
                                    <motion.div
                                        ...
                                        className="relative rounded-xl overflow-hidden aspect-square bg-gradient-to-br from-[#0a1128] to-[#0b1b42] border border-[#d4af37]/20 flex flex-col items-center justify-center cursor-pointer hover:border-[#d4af37]/40 transition-all group shadow-sm"
                                    >
                                        <motion.span
                                        ...
                                        >+9</motion.span>
                                        <span className="text-gray-400 text-[0.62rem] font-medium">more</span>
                                    </motion.div>
                                </motion.div>
                            )}
                        </motion.div>
