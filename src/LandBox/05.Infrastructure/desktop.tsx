          ...
          {infrastructureData.items.map((item, idx) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`w-9 h-9 rounded-[4px] flex items-center justify-center shrink-0 shadow-sm group-hover:animate-icon-shake origin-center transition-all duration-300 ${item.iconBg}`}
                >
              ...
              <span
                className={`text-[0.65rem] font-semibold tracking-[0.1em] px-3.5 py-1.5 rounded-[4px] shadow-sm uppercase ${getStatusStyle(item.status).replace("border-", "border-transparent ")}`}
              >
                {item.status}
              </span>
            </motion.div>
