    ...
    >
      ...
      >
        ...
        <div className="px-5 mt-5 mb-1 flex w-full">
          <div className="flex w-full bg-white/70 backdrop-blur-xl rounded-[4px] p-1.5 border border-gray-200/80 shadow-[0_8px_30px_rgb(0,0,0,0.06)] relative overflow-x-auto scrollbar-hide">
            {mediaData.tabs.map((tab) => (
              <button
                key={tab.label}
                onClick={() => setActiveTab(tab.label)}
                className={`relative flex-1 flex items-center justify-center gap-1.5 py-2 px-3 text-[0.75rem] font-bold whitespace-nowrap transition-all duration-300 rounded-[4px] z-10 focus-visible:outline-none min-w-max ${
                  activeTab === tab.label
                    ? "text-white"
