          ...
          >
            ...
            {commercialData.tabs.map((tab) => (
              <button
                ...
                className={`flex-1 relative flex flex-col items-center justify-center py-3 px-2 rounded-[4px] transition-all duration-300 z-10 group focus-visible:outline-none border ${
                  activeTab === tab
                    ? "border-transparent"
              ...
              <div className="flex flex-wrap gap-4">
                {currentData.details.map((detail, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5 text-gray-600">
                      <div className="relative w-8 h-8 rounded-[4px] flex items-center justify-center text-white shrink-0 shadow-sm overflow-hidden">
                        <motion.div
                          ...
                          className={`absolute inset-0 ${"bgClass" in detail ? detail.bgClass : "bg-[#17274c]"}`}
                        />
                        <detail.icon
                          size={15}
                          strokeWidth={2}
                    ...
                    <span
                      className={`text-[1rem] font-semibold tracking-tight leading-snug ${"valueColor" in detail ? detail.valueColor : "text-[#17274c]"}`}
                    >
                      {detail.value}
                    </span>
                  </motion.div>
