import React, { forwardRef, useEffect, type ButtonHTMLAttributes, type ReactNode, type HTMLAttributes } from 'react'
...
export const AnimatedCounter = forwardRef<HTMLSpanElement, AnimatedCounterProps>(
  ({ end, duration = 2000, prefix = '', suffix = '', className = '', ...props }, ref) => {
...
export type BadgeVariant = 'default' | 'success' | 'warning' | 'info' | 'accent' | 'error'
...
export const Badge = forwardRef<HTMLSpanElement, BadgeProps>(
  ({ children, variant = 'default', icon, pulse = false, className = '', ...props }, ref) => {
    return (
      <span
        ref={ref}
...
const buttonVariantStyles: Record<ButtonVariant, string> = {
  primary: 'gradient-accent text-cremp-navy font-bold shadow-elevation-2 hover:shadow-glow-accent hover:brightness-110 active:brightness-95',
  secondary: 'bg-cremp-primary text-white font-bold shadow-elevation-2 hover:bg-cremp-primary-light hover:shadow-glow-primary active:bg-cremp-primary',
  outline: 'bg-transparent border border-cremp-border text-cremp-text-primary font-semibold hover:bg-cremp-surface-alt hover:border-cremp-text-muted active:bg-cremp-border/30',
  ghost: 'bg-transparent text-cremp-text-secondary font-semibold hover:bg-cremp-surface-alt hover:text-cremp-text-primary active:bg-cremp-border/30',
}
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ variant = 'primary', size = 'md', icon, iconRight, loading = false, fullWidth = false, children, className = '', disabled, ...props }, ref) => {
    return (
      <button
        ref={ref}
...
export const Card = forwardRef<HTMLDivElement, CardProps>(
  ({ children, variant = 'default', hover = false, header, footer, className = '', padding = 'md', ...props }, ref) => {
    return (
      <div
        ref={ref}
        ...
        {...props}
      >
        {header && <div className="px-6 py-4 border-b border-cremp-border/50">{header}</div>}
        <div className={cardPaddingStyles[padding]}>{children}</div>
        {footer && <div className="px-6 py-4 border-t border-cremp-border/50">{footer}</div>}
      </div>
    )
  }
)
...
export const EmptyState = forwardRef<HTMLDivElement, EmptyStateProps>(
  ({ icon, title, description, action, className = '', ...props }, ref) => {
    return (
      <div ref={ref} className={`flex flex-col items-center justify-center py-16 px-6 text-center ${className}`} {...props}>
        ...
        {description && <p className="text-sm text-cremp-text-secondary max-w-sm mb-6">{description}</p>}
...
export const FeatureCard = forwardRef<HTMLDivElement, FeatureCardProps>(
  ({ icon, title, description, accentColor = 'text-cremp-primary', className = '', ...props }, ref) => {
    return (
      <div ref={ref} className={`group bg-white border border-cremp-border rounded p-6 hover-lift cursor-default ${className}`} {...props}>
        <div className={`w-12 h-12 rounded flex items-center justify-center mb-4 transition-base group-hover:scale-110 ${accentColor} bg-cremp-surface-alt`}>
          {icon}
        </div>
...
export const LoadingCard = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className = '', ...props }, ref) => (
    <div ref={ref} className={`bg-white border border-cremp-border rounded p-6 ${className}`} aria-busy="true" aria-label="Loading content" {...props}>
...
export const LoadingHero = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className = '', ...props }, ref) => (
    <div ref={ref} className={`w-full min-h-[70vh] flex flex-col justify-center px-6 ${className}`} aria-busy="true" aria-label="Loading hero section" {...props}>
...
export const LoadingStats = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement> & { count?: number }>(
  ({ count = 4, className = '', ...props }, ref) => (
    <div ref={ref} className={`grid grid-cols-2 md:grid-cols-4 gap-4 ${className}`} aria-busy="true" aria-label="Loading statistics" {...props}>
...
export const LoadingTimeline = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement> & { count?: number }>(
  ({ count = 3, className = '', ...props }, ref) => (
    <div ref={ref} className={`space-y-6 ${className}`} aria-busy="true" aria-label="Loading timeline" {...props}>
      {Array.from({ length: count }).map((_, i) => (
...
export const ProgressBar = forwardRef<HTMLDivElement, ProgressBarProps>(
  ({ value, max = 100, label, showPercentage = true, variant = 'primary', size = 'md', className = '', animated = true, ...props }, ref) => {
      ...
      <div ref={ref} className={className} {...props}>
        {(label || showPercentage) && (
          <div className="flex items-center justify-between mb-2">
            ...
            {showPercentage && <span className="text-xs font-bold text-cremp-text-primary">{Math.round(percentage)}%</span>}
          </div>
        )}
        <div
...
export const SearchBar = forwardRef<HTMLInputElement, SearchBarProps>(
  ({ placeholder = 'Search properties, franchises, opportunities...', className = '', containerClassName = '', showShortcut = true, ...props }, ref) => {
    return (
      <div className={`relative group ${containerClassName}`}>
        <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
          <Search size={18} className="text-cremp-text-muted group-focus-within:text-cremp-primary transition-colors duration-200" aria-hidden="true" />
        </div>
        <input
          ref={ref}
          type="search"
...
export const SectionHeader = forwardRef<HTMLDivElement, SectionHeaderProps>(
  ({ badge, badgeVariant = 'primary', title, subtitle, align = 'left', className = '', ...props }, ref) => {
    return (
      <div ref={ref} className={`mb-10 ${align === 'center' ? 'text-center' : ''} ${className}`} {...props}>
        {badge && (
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold tracking-widest uppercase mb-4 ${sectionHeaderBadgeStyles[badgeVariant]}`}>
            {badge}
          </span>
        )}
...
export const StatCard = forwardRef<HTMLDivElement, StatCardProps>(
  ({ icon, label, value, prefix = '', suffix = '', trend, className = '', ...props }, ref) => {
    return (
      <div ref={ref} className={`bg-white border border-cremp-border rounded p-5 hover-lift group ${className}`} {...props}>
        <div className="flex items-center justify-between mb-3">
          ...
          {trend && (
            <span className={`inline-flex items-center gap-0.5 text-xs font-bold ${trend.positive ? 'text-success' : 'text-error'}`}>
...
export const Timeline = forwardRef<HTMLDivElement, TimelineProps>(
  ({ items, className = '', ...props }, ref) => {
    return (
      <div ref={ref} className={`space-y-0 ${className}`} role="list" aria-label="Timeline" {...props}>
        {items.map((item, index) => (
          <div key={index} className="flex gap-4 group" role="listitem">
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${timelineStatusColors[item.status || 'pending']} transition-base`}>
                {item.icon}
              </div>
              {index < items.length - 1 && (
                <div className={`w-0.5 flex-1 my-1 ${timelineLineColors[item.status || 'pending']}`} />
              )}
            </div>
            <div className="flex-1 pb-6">
