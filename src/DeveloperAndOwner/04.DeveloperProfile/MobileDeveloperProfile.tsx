...
import { DeveloperProfileData } from './data';
...
export default function MobileDeveloperProfile() {
              ...
              <motion.div key={idx} variants={fadeInUp} className="overflow-hidden rounded-[8px] border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-[#0a101d]">
                ...
                >
                  <div className="flex items-center gap-3">
                    <pill.icon size={20} className={isActive ? 'text-white/80 dark:text-gray-900/80' : 'text-[#D4AF37] dark:text-[#D4AF37]'} />
                ...
                <AnimatePresence>
                  {isActive && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-gray-50 dark:bg-[#0a1128]">
                      <div className="p-4 flex flex-col gap-4">
                        <div className="flex items-center gap-2 mb-2">
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#D4AF37]/10 text-[#D4AF37] dark:bg-[#D4AF37]/10 dark:text-[#D4AF37]">
                            {(() => {
                              const FirstIcon = DeveloperProfileData.expectations[0].icon;
                              return <FirstIcon size={14} />;
                            })()}
                          </div>
                          <span className="font-bold text-gray-900 dark:text-white text-[0.85rem] uppercase tracking-wide">{DeveloperProfileData.expectationsTitle}</span>
                        </div>
                        {DeveloperProfileData.expectations.map((exp, eIdx) => (
                          <div key={eIdx} className="flex gap-3 items-start">
                            <exp.icon size={16} className="text-[#D4AF37] mt-1 shrink-0 dark:text-[#D4AF37]" />
