        ...
        <div className="flex items-center justify-between gap-16">
          <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} className="w-1/2">
              ...
              {testimonialsData.title.split('\n').map((line, idx) => (
                <span key={idx}>
                  {idx === testimonialsData.title.split('\n').length - 1 ? (
                    <span className="bg-gradient-to-r from-[#D4AF37] to-[#b38728] bg-clip-text text-transparent dark:from-[#D4AF37] dark:to-[#f9d08b]">{line}</span>
                  ) : (
                    <>{line}<br /></>
                  )}
                </span>
            ...
            <motion.div variants={fadeInUp} className="flex items-center gap-4">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center gap-3 rounded-[8px] bg-gradient-to-r from-[#D4AF37] to-[#b38728] px-8 py-4 text-[0.95rem] font-bold text-white shadow-lg dark:from-[#D4AF37] dark:to-[#b38728] dark:text-[#030712]">
                <Btn1Icon size={20} />
              ...
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center gap-3 rounded-[8px] border border-gray-200 bg-white px-8 py-4 text-[0.95rem] font-bold text-gray-900 shadow-sm transition-all hover:bg-gray-50 dark:border-gray-800 dark:bg-[#0a101d] dark:text-white dark:hover:bg-gray-900/50">
                <Btn2Icon size={20} className="text-[#D4AF37] dark:text-[#D4AF37]" />
            ...
            <div className="relative rounded-[8px] bg-gray-50 p-10 shadow-inner dark:bg-[#0a101d] border border-gray-100 dark:border-gray-800 transition-all duration-300 hover:shadow-lg hover:border-[#D4AF37]/50">
              ...
              <div className="flex flex-col gap-4">
                {testimonialsData.benefits.map((benefit, idx) => (
                  <motion.div key={idx} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 + idx * 0.1 }} className="flex items-center gap-4 rounded-[8px] bg-white p-5 shadow-sm dark:bg-[#0a1128] border border-gray-100 dark:border-gray-800 transition-all duration-300 hover:shadow-lg hover:border-[#D4AF37]/50">
