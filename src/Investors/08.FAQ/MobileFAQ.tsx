  ...
  return (
    <div className="relative w-full overflow-hidden bg-gray-50 px-4 py-16 transition-colors duration-700 dark:bg-[#030712]">
      <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: true }} className="relative z-10 flex flex-col">
            ...
            return (
              <div key={idx} className="border-b border-gray-200 dark:border-gray-800">
                <button type="button" onClick={() => setOpenIdx(isOpen ? null : idx)} className="group flex w-full items-center justify-between py-5 text-left">
                  <h4 className={`text-[0.95rem] font-bold pr-4 transition-colors ${isOpen ? 'text-[#D4AF37] dark:text-[#D4AF37]' : 'text-gray-900 dark:text-gray-300'}`}>{faq.q}</h4>
                  <div className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full transition-all ${isOpen ? 'bg-gradient-to-r from-[#FEF08A] via-[#FBBF24] to-[#F59E0B] text-white dark:bg-gradient-to-r from-[#FEF08A] via-[#FBBF24] to-[#F59E0B] dark:text-gray-900' : 'bg-gray-100 text-gray-400 dark:bg-gray-800'}`}>
        ...
        <motion.div variants={fadeInUp} className="flex flex-col items-center justify-center gap-6 rounded-[8px] bg-[#0a1128] p-8 text-center shadow-xl relative overflow-hidden">
