...
export default function DesktopFAQ() {
  ...
  const leftColFaQS = faqData.faqs.slice(0, midPoint).map((faq, i) => ({ ...faq, originalIdx: i }));
  const rightColFaQS = faqData.faqs.slice(midPoint).map((faq, i) => ({ ...faq, originalIdx: i + midPoint }));
  ...
  const renderFaqCard = (faq: any) => {
    ...
    return (
      <motion.div 
        ...
        viewport={{ once: true }}
        whileHover={{ y: -2 }}
        className={`group relative overflow-hidden rounded-[4px] border bg-white/60 p-6 shadow-sm backdrop-blur-md transition-all hover:shadow-lg dark:bg-gray-900/40 ${
          isOpen 
      ...
      >
        ...
        >
          <div className={`mt-0.5 flex h-12 w-12 shrink-0 items-center justify-center rounded-full border transition-colors duration-300 ${
            isOpen 
            ...
            <faq.icon size={20} strokeWidth={isOpen ? 2.5 : 1.5} className="transition-transform duration-500 group-hover:rotate-12 group-hover:scale-110" />
          </div>
          
          <div className="flex flex-1 flex-col pt-1">
            <div className="flex items-center justify-between gap-4">
              <h4 className={`text-lg font-bold leading-tight transition-colors duration-300 ${
                isOpen ? 'text-[#B27F1C] dark:text-[#F6B23B]' : 'text-gray-900 dark:text-white'
              }`}>
                {faq.q}
              </h4>
              <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-colors ${isOpen ? 'bg-[#B27F1C]/10 dark:bg-[#F6B23B]/10' : 'bg-gray-50 dark:bg-gray-800/50 group-hover:bg-gray-100 dark:group-hover:bg-gray-800'}`}>
                <ChevronDown 
                  size={18} 
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-[8px] bg-gray-50 pt-32 py-4 transition-colors duration-700 dark:bg-[#030712] dark:shadow-none">
      ...
      <Container className="relative z-10 flex flex-col items-center">
        ...
        <motion.div 
          ...
          className="mb-20 flex max-w-3xl flex-col items-center text-center"
        >
          ...
          <motion.h2 variants={fadeInUp} className="mb-6 text-[3rem] font-black leading-[1.05] tracking-tight text-gray-900 dark:text-white sm:text-[3.5rem] lg:text-[4rem]">
            {faqData.title}
          </motion.h2>

          <motion.p variants={fadeInUp} className="text-lg font-medium leading-relaxed text-gray-600 dark:text-gray-400">
            {faqData.desc}
          </motion.p>
        </motion.div>

        <div className="mb-24 grid w-full grid-cols-1 items-start gap-6 lg:grid-cols-2 lg:gap-8 xl:px-8">
        ...
        >
          ...
          <div className="absolute right-0 top-0 h-full w-1/2 opacity-[0.03]">
            <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-full w-full">
              ...
              <defs>
                <linearGradient id="banner-grad" x1="0" y1="0" x2="200" y2="200" gradientUnits="userSpaceOnUse">
