      ...
      <Container className="relative z-10">
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          <div className="w-[35%] flex flex-col pt-4">
            ...
            >
              <p className="text-[1.1rem] text-[#4b5563] font-medium leading-relaxed">{investorData.desc[0]}</p>
              <p className="text-[1.1rem] text-[#0a1128] font-bold leading-relaxed max-w-sm">{investorData.desc[1]}</p>
            </motion.div>
          </div>
          ...
          <div className="w-[60%] relative h-[450px]">
             ...
             >
               ...
               <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 bg-white rounded-xl shadow-2xl p-5 border border-[#e5e7eb] scale-110">
                  <div className="flex items-center justify-between mb-4 border-b border-[#e5e7eb] pb-3">
                     <span className="font-bold text-[#0a1128] text-sm">Qualified Investor Enquiry</span>
                     <span className="bg-[#166534] text-white text-[0.6rem] font-bold px-2 py-1 rounded shadow-sm">★ High Match</span>
                  </div>
                  
                  <div className="flex gap-6">
                     <div className="flex flex-col items-center">
                        <span className="text-[0.6rem] font-bold text-[#6b7280] mb-2">Match Score</span>
                        {/* Circular Progress */}
                        <div className="w-16 h-16 rounded-full border-4 border-[#e5e7eb] border-t-[#166534] border-r-[#166534] border-b-[#166534] flex items-center justify-center rotate-45 relative">
                     ...
                     <div className="flex flex-col flex-1">
                        <span className="text-[0.6rem] font-bold text-[#6b7280] mb-2">Key Highlights</span>
                        <div className="flex flex-col gap-1.5">
                           <div className="flex items-center gap-1.5 text-[0.65rem] font-bold text-[#0a1128]">
             ...
             <motion.div 
               ...
               className="absolute right-0 top-1/2 -translate-y-1/2 bg-white/95 backdrop-blur-md rounded-2xl shadow-xl border border-[#e5e7eb] p-4 flex flex-col gap-4 z-20"
             >
                {investorData.pills.map((pill, idx) => (
                  <div key={idx} className="flex items-center gap-3">
                     <div className="w-8 h-8 rounded-full bg-[#fdf8f0] flex items-center justify-center shrink-0 border border-[#f6ead6] text-[#d97706]">
                       <pill.icon size={14} strokeWidth={2} />
                     </div>
                     <span className="text-[0.75rem] font-bold text-[#4b5563] whitespace-nowrap pr-2">
                       {pill.label}
                     </span>
                  </div>
                ))}
        ...
        <div className="w-full bg-[#fdf8f0] rounded-[2rem] border border-[#f6ead6] flex overflow-hidden shadow-sm relative">
           
           {/* Left Half (Grid) */}
           <div className="w-[65%] p-10 pr-16 bg-[#fcf9f2]">
              <div className="flex items-center gap-4 mb-8">
                 <div className="w-12 h-12 rounded-full bg-[#0a1128] flex items-center justify-center text-white shrink-0 shadow-md">
                 ...
                 {investorData.expectations.map((item, idx) => (
                   <div key={idx} className="flex gap-4">
                      <div className="flex flex-col items-center shrink-0">
                         ...
                         <div className="w-[2px] h-10 bg-[#e5e7eb]" />
                      </div>
                      <div className="flex flex-col pt-0.5">
                         <div className="flex items-center gap-2 mb-2">
                            <item.icon size={20} className="text-[#0a1128]" strokeWidth={1.5} />
                            <span className="font-bold text-[#0a1128] text-[0.95rem]">{item.title}</span>
                         </div>
                         <p className="text-[0.75rem] text-[#6b7280] font-medium leading-relaxed">
                            {item.desc}
                         </p>
                      </div>
                   </div>
           ...
           {/* Right Half (Outcome) */}
           <div className="w-[35%] p-10 bg-[#fdfdfd] relative flex flex-col justify-center">
              ...
              >
                 ...
                 <h3 className="text-[1.8rem] font-black text-[#0a1128] leading-[1.1] mb-10">
