...
export default function MobileYourMarkets() {
  return (
    <div className="w-full bg-[#fdfdfd] pt-12 pb-12 px-5 overflow-hidden relative">
      ...
      >
        <p className="text-[0.95rem] text-[#0a1128] font-bold leading-relaxed">{marketData.desc[0]}</p>
        <p className="text-[0.9rem] text-[#4b5563] font-medium leading-relaxed">{marketData.desc[1]}</p>
      </motion.div>

      {/* Hexagon Map (Mobile friendly stack) */}
      <div className="w-full relative py-6 mb-10 flex flex-col items-center justify-center bg-gray-50 rounded-2xl border border-[#e5e7eb] overflow-hidden">
         {/* Map Background */}
         ...
         {/* Connecting Line */}
         <div className="absolute top-10 bottom-10 w-[2px] border-l-[2px] border-dashed border-[#9ca3af]" />
              ...
              return (
                ...
                >
                   <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-[#0a1128] text-[0.8rem]">{hex.title}</span>
                   </div>
      ...
      {/* Benefits Section */}
      <div className="w-full bg-white rounded-[1.5rem] border border-[#e5e7eb] p-6 shadow-sm mb-6">
         <div className="inline-flex items-center gap-2 bg-[#0a1128] text-white px-3 py-1.5 rounded-full mb-6 shadow-sm">
         ...
         <div className="flex flex-col gap-4">
            {marketData.benefits.map((benefit, idx) => (
              <div key={idx} className="flex items-center gap-3">
                 <div className="w-8 h-8 rounded-full bg-[#fdf8f0] flex items-center justify-center text-[#d97706] shrink-0 border border-[#f6ead6]">
      ...
      <motion.div 
         ...
         className="w-full p-6 bg-[#fdf8f0] rounded-2xl border border-[#f6ead6] relative flex flex-col justify-center items-center text-center shadow-sm overflow-hidden"
      >
         ...
         <div className="relative w-full h-48 flex items-center justify-center">
            {/* Map Base */}
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-48 h-24 bg-white rounded-xl shadow-md border border-[#e5e7eb] transform rotate-[-15deg] skew-x-[20deg] flex overflow-hidden">
               <div className="w-full h-full bg-[linear-gradient(to_right,#e5e7eb_1px,transparent_1px),linear-gradient(to_bottom,#e5e7eb_1px,transparent_1px)] bg-[size:15px_15px]" />
            </div>
            ...
            <div className="absolute bottom-8 right-[10%] flex items-end gap-1.5 z-10">
               ...
               <div className="w-6 h-24 bg-[#0a1128] rounded-t-sm shadow-sm" />
            </div>
