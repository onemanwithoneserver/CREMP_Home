      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 shrink-0 bg-white dark:bg-[#0b1b42] dark:border-white/10 border-b border-black/[0.06]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[12px] font-bold text-[#0a1128] dark:text-white font-['Outfit',sans-serif]">
          ...
          <button 
            ...
            className="w-7 h-7 rounded-lg border border-black/[0.08] flex items-center justify-center hover:bg-[#f5f6f8] transition-colors bg-white dark:bg-[#0b1b42] dark:border-white/10"
          >
        ...
        <div className="flex-1 relative overflow-hidden">
            ...
            {(Object.entries(MAP_PINS_BY_TIER) as [keyof typeof MAP_PINS_BY_TIER, { x: number; y: number }[]][]).map(([tier, pins]) =>
              pins.slice(0, tierCounts[tier]).map((pin, i) => {
                ...
                return (
                  ...
                  >
                    <div 
                      className="px-2 py-0.5 rounded-md text-[10px] font-bold shadow whitespace-nowrap bg-white dark:bg-[#0b1b42] dark:border-white/10 text-[#0a1128] dark:text-white font-['Outfit',sans-serif] border-[1.5px] border-solid" 
          ...
          <div className="absolute top-3 right-3 flex flex-col gap-1 z-10">
            <button 
              ...
              aria-label="Zoom in" 
              className="w-7 h-7 bg-white dark:bg-[#0b1b42] dark:border-white/10 rounded flex items-center justify-center shadow-sm border border-black/[0.08] hover:bg-[#f5f6f8] transition-colors"
            >
