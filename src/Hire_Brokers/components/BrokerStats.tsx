...
function Stat({ label, value }: StatItem) {
  return (
    <div className="flex flex-col items-center gap-0.5">
      <span className="text-[13px] font-bold text-[#0a1128] dark:text-white leading-tight">{value}</span>
    ...
    return (
      <div className={`grid grid-cols-3 gap-2 ${className}`}>
        {stats.map((s) => (
          <div
            key={s.label}
            className="flex flex-col items-center bg-[#fafafb] dark:bg-background rounded-lg py-2 px-1 border border-black/[0.05]"
          >
            <Stat {...s} />
          </div>
        ))}
